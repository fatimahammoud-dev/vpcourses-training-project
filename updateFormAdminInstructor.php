...
$id = $_GET['id'];
require('connect.php');
$sql = "SELECT * from admininstructor WHERE id = '$id' ";
...
if ($_POST) {
    if ($_FILES && $_FILES['images']['name']) {
        ...
        $target_dir = "vpCourses/img/instructors/";
        ...
        if ($result) {
            $sql = "UPDATE admininstructor SET image='$image_name' WHERE id = " . $instructors['id'];
            $conn->query($sql);
        }
   } elseif (isset($_POST['deleteImage'])) {
    $imageToDelete = basename($_POST['imageNameToDelete'] ?? '');
    $pathToDelete = __DIR__ . "/vpCourses/img/instructors/" . $imageToDelete;

    if ($imageToDelete !== '' && is_file($pathToDelete)) {
        unlink($pathToDelete);
    }

    $sql = "UPDATE admininstructor SET image=NULL WHERE id = " . $instructors['id'];
    $conn->query($sql);
}
}
?>
                ...
                <div class="col-12">
                    <div class="bg-light rounded h-100 p-4">
                        <div class="row">
                            ...
                            $sql = "SELECT image FROM admininstructor WHERE id = " . $instructors['id'];
                            $result = $conn->query($sql);
                            ...
                            if ($imageName) {
                                echo "<div>";
                                echo "<img class='' src='vpCourses/img/instructors/$imageName' alt='Uploaded Image' width='100px' ><br>";
                            ...
                            <form action="updateAdminInstructor.php" method="post" class="mt-4">

                                <div class="input-group mb-3" hidden>
                                    <span class="input-group-text w-120 text-center" style="width: 130px !important;">ID</span>
                                    <input type="text" class="form-control" id="userID" name="id" value="<?php echo $instructors['id'] ?>" required>
                                </div>
                                <div class="input-group mb-3">
                                    <span class="input-group-text w-120 text-center" style="width: 130px !important;">Course Name</span>
                                    <input type="text" class="form-control" id="#" name="course" value="<?php echo $instructors['course'] ?>" required>
                                </div>
                                <div class="input-group mb-3">
                                    <span class="input-group-text w-120 text-center" style="width: 130px !important;">instructor Name</span>
                                    <input type="text" class="form-control" id="#" name="instructor" value="<?php echo $instructors['name'] ?>" required>
                                </div>
