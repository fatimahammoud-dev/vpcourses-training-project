...
$id = $_GET['id'];
require('connect.php');
$sql = "SELECT * from adminstudent WHERE id = '$id' ";
...
if ($_POST) {
    if ($_FILES && $_FILES['images']['name']) {
        ...
        $target_dir = "vpCourses/img/students/";
        ...
        if ($result) {
            $sql = "UPDATE adminstudent SET std_image='$image_name' WHERE id = " . $students['id'];
            $conn->query($sql);
        }
    } elseif (isset($_POST['deleteImage'])) {
    $imageToDelete = basename($_POST['imageNameToDelete'] ?? '');
    $pathToDelete = __DIR__ . "/vpCourses/img/students/" . $imageToDelete;

    if ($imageToDelete !== '' && is_file($pathToDelete)) {
        unlink($pathToDelete);
    }

    $sql = "UPDATE adminstudent SET std_image=NULL WHERE id = " . $students['id'];
    $conn->query($sql);
}
}
?>
                ...
                <div class="col-12">
                    <div class="bg-light rounded h-100 p-4">
                        <div class="row">
                            ...
                            $sql = "SELECT std_image FROM adminstudent WHERE id = " . $students['id'];
                            $result = $conn->query($sql);

                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    $imageName = $row['std_image'];
                                }
                            }

                            if ($imageName) {
                                echo "<div>";
                                echo "<img class='' src='vpCourses/img/students/$imageName' alt='Uploaded Image' width='100px' ><br>";
                            ...
                            <form action="updateAdminStudent.php" method="post" class="mt-4">

                                <div class="input-group mb-3" hidden>
                                    <span class="input-group-text w-120 text-center" style="width: 130px !important;">ID</span>
                                    <input type="text" class="form-control" id="userID" name="id" value="<?php echo $students['id'] ?>" required>
                                </div>
                                <div class="input-group mb-3">
                                    <span class="input-group-text w-120 text-center" style="width: 130px !important;">Student Name</span>
                                    <input type="text" class="form-control" id="#" name="name" value="<?php echo $students['name'] ?>" required>
                                </div>
                                <div class="input-group mb-3">
                                    <span class="input-group-text w-120 text-center" style="width: 130px !important;">Comment</span>
                                    <input type="text" class="form-control" id="#" name="comment" value="<?php echo $students['comment'] ?>" required>
                                </div>
